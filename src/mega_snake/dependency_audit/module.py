...
from mega_snake.dependency_audit.scanner import scan_dependencies, Vulnerability
...
@click.command(
    ...
    help="""Scans the project's locked dependencies (`uv.lock`) for known vulnerabilities using
    `pip-audit` and the OSV advisory database, then files a GitHub issue for each new finding
    (package, installed version, recommended version, severity and advisory link), skipping
    findings that were already reported.""",
    epilog="""
...
def scan_dependencies_command(dry_run: bool) -> None:
    ...
    vulnerabilities: list[Vulnerability] = scan_dependencies()
