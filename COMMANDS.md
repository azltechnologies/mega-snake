...
Generates SKILL.md for AI agent skill discovery by introspecting the registered CLI commands and rendering the same reference as generate-docs, behind the YAML frontmatter the agent runtimes read. Asks which assistant to target — .github/skills/mgsnake/ for GitHub Copilot, .claude/skills/mgsnake/ for Claude, or both — and how the files should be tracked in git, then writes them.
