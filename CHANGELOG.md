<!-- markdownlint-disable MD024 -->
<!-- Keep a Changelog repeats `### Added` / `### Changed` / `### Fixed` under every version by
     design, so the duplicate-heading rule does not apply to this file. -->

# Changelog

All notable changes to `mega-snake` are documented here.

The format follows [Keep a Changelog](https://keepachangelog.com/en/1.1.0/), and this project adheres to
[Semantic Versioning](https://semver.org/spec/v2.0.0.html).

This file is written by hand, on purpose. It is read by people who install `mgsnake` and want to know what
changed for *them* — not by people reading the commit log. The exhaustive, auto-generated list of commits
lives in the GitHub Release body (`gh release create --generate-notes`); this file is the curated half.

**Releasing:** every release must have its own `## [X.Y.Z]` section here, matching the `version` field in
`pyproject.toml` and the `vX.Y.Z` tag. `.github/workflows/release.yml` refuses to publish otherwise.

## [0.1.7] - 2026-08-25

### Added

- **`mgsnake config`.** A persistent settings store the CLI both writes and reads, replacing three
  configuration layers that could not be written to. Subcommands: `get KEY`, `set KEY VALUE
  [--global]`, `unset KEY [--global]`, `list [--scope repo|global|all]` and `export [--shell]
  [--scope]`. State lives in `<git-dir>/mgsnake/state.json` (repo scope, per-clone) and
  `~/.config/mgsnake/state.json` / `%APPDATA%\mgsnake\state.json` (global scope), written
  atomically. Reads resolve **environment variable → repo → global**. Credential-shaped keys
  (`token`, `secret`, `password`, `passwd`, `credential`, `api_key`) are refused on both `set` and
  `get`. A corrupt state file offers to back itself up and reset on an explicit scope from an
  interactive terminal, and fails loudly everywhere else; a merged read (`get`, `list --scope all`,
  `export --scope all`) degrades with a warning instead of blocking a setting that lives in the
  healthy scope.
- **`mgsnake jira-board`, `jira-sprint` and `jira-issues`.** Native replacements for
  `getJiraBoard.sh`, `getSprintInfo.sh` and `jira_board_issues.sh`, built on the new `config` store
  so a project key only needs to be set once (`mgsnake config set jira.project_key TAROTAPP`).
  - `jira-board` resolves a project key to its board id and caches it per clone (`--refresh` to
    force re-resolution); a project with several boards prompts once and caches the answer.
  - `jira-sprint` lists the board's active sprint(s) as a JSON array.
  - `jira-issues` downloads the whole board to `workspace_temp/jira_board_issues.json` (or
    `--output`) in one request-light pass, paging through Jira's Agile API to the end so no sprint
    hides on a later page. Story points and sprint custom field ids are resolved by display name
    and cached; ambiguous or unresolved names fall back to a warning and are never cached, so the
    warning keeps appearing until the field is pinned with
    `mgsnake config set jira.field.sprint customfield_10020` (undo with `config unset`).
  - Retry and pooling are handled by `urllib3`/`requests.Session`; only `GET` is retried, and
    401/403/404 never are. Requires `jira.domain`, `jira.email` and `JIRA_API_TOKEN`
    (`JIRA_MCP_TOKEN` is a deprecated fallback that warns on stderr); `REQUESTS_CA_BUNDLE` is
    honored for TLS-inspecting corporate proxies.
- **`working-env` now configures only the stacks a repository actually uses.** A new `ProjectStack`
  model tags every workspace artifact (tasks, launch configurations, log watchers, inputs,
  recommended extensions) with the stack it belongs to, so a repository with no JVM code no longer
  gets asked for a JDK or has Java/Gradle/Maven tasks, launch configs and settings written into its
  workspace. `--stack/-s` (repeatable, `all` supported) overrides the detection for layouts it
  cannot see, such as a build file in a subfolder.

### Changed

- Skipped stacks (e.g. no Gradle or Maven file found) are now reported once, through a single
  informational message, instead of one warning per stack on every run of `working-env`.

### Fixed

- `LogWatcher.GENERIC` moved to the `python` stack: it only watches a log file the Python launch
  configurations produce, so Java-only and Node-only workspaces were previously given a watcher for
  a file they never generate.
- **`generate-docs` no longer depends on the terminal that ran it.** Click wrapped each command's
  usage line to the current terminal width, so `COMMANDS.md` came out different on a narrow screen
  than on CI — and a narrow enough terminal even split a metavar mid-word. The synopsis is now
  rendered at a fixed width and collapsed to a single line, so the committed reference matches the
  `generate-docs --check` run in the release workflow.

### Breaking

- `jira-board`'s `boardId` is now a JSON number rather than a string. `jira-sprint` now always
  returns a JSON array, including when there is a single active sprint (the shell version emitted a
  bare object in that case, and two concatenated objects when there were two).

## [0.1.6] - 2026-08-24

### Added

- **`working-env --stack`.** Overrides the stack detection when the layout does not reveal what a
  repository is: `-s python -s node` configures exactly those, and `-s all` forces every stack. The
  option replaces the detection entirely rather than adding to it, and only the current directory is
  ever inspected, so a vendored `package.json` in a subfolder cannot pull Node tooling in.
- **`generate-docs` and `man`.** The command reference (`COMMANDS.md`) is now generated by introspecting the
  live Click objects and appending the hand-written fragments in `resources/docs/`, so the synopsis, options,
  defaults and aliases can no longer drift from the code. `man` pages the same reference in the terminal,
  either whole or for a single command, and works on PowerShell where `man` does not exist.
- **`diff-tree --scope`.** Chooses how much of the work to include: committed only (default), committed and
  staged, or also unstaged and untracked. Untracked files are reported as additions, and pending work is
  listed above the commit list.
- **`diff-tree --target-hash`.** Moves the far end of the comparison off `HEAD`, so a past range can be
  inspected by naming both of its commits. Refused for the staged and unstaged scopes, which only exist for
  `HEAD`.
- **Automated release pipeline.** Publishing to PyPI now runs from a GitHub Release, gated on the tag
  matching `vX.Y.Z`, the version in `pyproject.toml` agreeing with it, and this file documenting the release.
  `twine check` validates the distributions before the upload.
- **`create-release --tag-pattern`.** The release tag format is no longer hard-coded to `vX.Y.Z`: a
  pattern such as `$1.$2.$3` or `rel-$1_$2_$3` describes whatever scheme the project already uses,
  where `$1`, `$2` and `$3` are the major, minor and patch numbers. Set it per invocation, or per
  project with the `release_tag_pattern` property. The pattern must match the latest release's tag.
- **`create-release --version-part`.** The new tag is now derived from the latest release by incrementing
  its `patch`, `minor` or `major` component, instead of appending a free-form suffix. `--tag-suffix` still
  produces pre-release builds (`v1.2.4-beta.0`), and is rejected for `latest` releases, which GitHub only
  grants to a plain version.
- **Automatic shell environment reload.** Configuring the environment (`working-env`, `set-java`, `set-gradle`,
  `set-maven`, `init-local-config`) now takes effect in your *current* shell session, without opening a new
  terminal. The shell init script defines `mgsnake` as a thin function around the real executable: the command
  reports it rewrote the environment, and the function re-sources the local files for you. Only the commands
  that actually touch those files trigger it, so `graphql-schema` and `maven-project-setup` no longer force a
  pointless reload. Once the function has reloaded for you the command reports success, so it still composes
  with `&&` and does not abort a `set -e` script.
- **`reload-config` and `load-env`.** Re-source the local configuration file, or export the variables declared in
  an environment file, into the shell session you are already in — no new terminal, no `export` per variable.
- **`local-config-path` and `local-env-path`.** Print the resolved path of the local configuration file or the
  local environment file (`.mgsnake.env`), so a shell profile or script can locate them without duplicating the
  resolution logic. (`local-config-path` was briefly named `get-local-config-path` earlier in this same
  unreleased cycle; nothing under the old name has shipped in a release.)
- **This changelog.**

### Changed

- **`working-env` now configures only the stacks your repository actually uses.** It detects them
  from the build files in the current directory — `build.gradle`/`settings.gradle` for Gradle,
  `pom.xml` for Maven, `pyproject.toml`/`uv.lock` for Python, `package.json`/`tsconfig.json` for
  Node — and writes only the extensions, tasks, launch configurations and log watchers that belong
  to them. **A repository with no JVM build file is no longer asked to pick a JDK or a Gradle
  distribution**, and no longer receives the Java and Gradle entries it can never run. Skipped
  stacks are reported once, with the command that configures each one on demand
  (`mgsnake set-java`, `set-gradle`, `set-maven`) — those stay unconditional, so nothing is lost
  when the detection does not match your layout. Use `--stack` to override it.

  **Nothing is ever removed from an existing `.code-workspace`.** Re-running on a workspace that was
  configured before this release keeps every extension, task and launch configuration already in it;
  narrowing the stacks only means less is added from now on.
- **`mgsnake` now reports a meaningful exit code instead of always exiting `1`.** Scripts and CI steps can tell
  a missing `osv-scanner` (`102`) from a bad revision (`103`), an unset `MEGA_SNAKE_SHELL` (`112`) from a
  mistyped flag (`1`), stale generated docs (`113`) from a declined prompt (`114`). The full table is in the
  contributor docs. Previously every one of these exited `1`.
- **Failures that are not your fault no longer look like misuse.** A repository without a remote, a missing
  external tool and a prompt you declined were all reported with the "you called this wrong" status; each now
  carries its own.

- **`diff-tree` renamed its revision options** to `--origin-hash` / `-o` (was `--commit-hash` / `-c`), so
  the two ends of the comparison read as a pair with `--target-hash`.
- `create-release` no longer takes a `tag_suffix` positional argument; the version is derived and the
  suffix is an option.
- The PyPI long description is now assembled with `hatch-fancy-pypi-readme` instead of shipping `README.md`
  verbatim.
- **Progress, status, warnings and errors now go to stderr**, not stdout. stdout carries only what a command
  *produces*, so `. "$(mgsnake shell-path bash)"` and `$(mgsnake local-config-path)` capture the value alone
  and `mgsnake <command> 2>/dev/null` keeps it. If you redirect stdout only (`mgsnake rbd > run.log`, or a CI
  step archiving stdout), add `2>&1` to keep the diagnostics. Previously only `ws_advice` was on stderr.
- **`remote-branches-details` and `remote-branches-cleanup` no longer require a remote.** Both now pair every
  local branch with its remote counterpart (through its upstream) into one logical branch, so a repository
  without a remote still gets its local branches reported and cleaned, and a local branch whose remote copy
  was already deleted after a merge — the most common form of dead branch, since `git fetch --prune` drops
  the tracking reference but leaves the local branch behind forever — is no longer invisible to either
  command. A branch counts as merged only when *every* side it exists on is independently merged.
- **`remote-branches-details` now writes `workspace_temp/remote_branches.md`** (was `remote_branches.txt`),
  with a richer table: merge status, tracking/sync state, both hashes side by side, last commit/author/subject,
  and the branch's common ancestor with main — plus a header recording the remote, main branch, filter and
  generation time the report was built from.
- **`remote-branches-cleanup` no longer reads `remote-branches-details`'s report.** It builds its own branch
  inventory in memory and acts on the repository as it is right now, instead of on a file that may be stale by
  the time you run the destructive step. A selected branch is deleted only from the side(s) it actually has —
  `git branch -D` locally, `git push <remote> --delete` remotely — rather than assuming both, so a branch
  missing one side no longer reports a spurious deletion failure for it.
- Both `remote-branches-details` and `remote-branches-cleanup` now offer to fetch and prune before building
  their branch inventory, so the report or the deletion prompts reflect the remote as it is now.
- **`run_operation` retries a network timeout instead of aborting on it.** A slow `git fetch` (cold cache, VPN,
  a credential prompt) used to raise a raw `TimeoutExpired` on the very first attempt, skipping the retries
  that exist precisely for transient failures; a timeout is now caught and retried the same as any other
  failed attempt.

### Removed

- **Breaking: the shell helpers are private now**, and the public interface is the CLI itself — `mgsnake_reload`
  is `mgsnake reload-config`, `mgsnake_load_env` is `mgsnake load-env`, and `mgsnake_reload_all` is gone
  outright (it loaded the `.env` of whatever directory you happened to be in, on top of the one the config file
  had already loaded by absolute path). No compatibility shim is kept.

  **If you generated a local config file before this release**, it calls `mgsnake_load_env` by name, and it is
  sourced on every shell startup — so you will get `mgsnake_load_env: command not found` in each new terminal
  and the environment file will stop loading. Fix the single line to call `__mgsnake_load_env` instead, or
  regenerate the file with `mgsnake init-local-config -o`. Anything of your own that called `mgsnake_reload`
  needs the same treatment.

### Fixed

- **Duplicate command and alias registrations are now rejected.** Click's registry silently replaced an
  existing command when a second one claimed its name, leaving the first unreachable while its tests kept
  passing. Registration now fails loudly, naming both colliding modules.
- **The shell environment reload stopped working** when `mgsnake` became an installed executable: the wrapper
  that captured its exit status was gone, so the signal was emitted into a void. Restored, and now tested
  against a real process instead of a hand-built object.
- **Exit codes never reached the shell.** The installed entry point called the command group directly, so the
  translation from exception to status — and the hook that delivers it — only ran under `python -m mega_snake`.
  Two more holes are closed with it: an internal error with no mapped type reported `1` instead of `100`, and
  an exception type that was not listed verbatim (`subprocess.CalledProcessError`, for instance) fell back to
  the generic code instead of inheriting its parent's.
- **`remote-branches-details` on an empty repository now reports its own exit code (`107`)** instead of the
  one also used for an invalid `--filter-by` (`103`), so a script can tell the two failures apart.

## [0.1.5] - 2026-07-26

### Added

- `--version` / `-v` on the root command.
- `scan-dependencies` became a multi-ecosystem audit engine: the ecosystem is auto-detected from the
  lockfiles (Python/uv, Java/Gradle/Maven, Node) with a generic OSV-Scanner fallback, and `--ecosystem`
  forces it.

### Fixed

- `diff-tree` crashed with a `UnicodeDecodeError` when a binary file appeared in the diff. Binary files are
  now detected and written with a placeholder instead of their raw bytes.
- `osv-scanner` failures are reported instead of being swallowed.

## [0.1.4] - 2026-07-24

### Added

- **`scan-dependencies` (`sdep`, `audit`).** Scans the project's locked dependencies against the OSV advisory
  database and files one GitHub issue per new finding — package, installed version, recommended version and
  advisory link — skipping findings that were already reported. `--dry-run` prints them instead. The issue
  labels are created if they do not exist yet.
- Dependabot configuration and a scheduled dependency-scan workflow.

### Fixed

- `gh issue list` and `pip-audit` no longer have stderr merged into their JSON output, which corrupted
  parsing whenever either tool printed a warning.

## [0.1.3] - 2026-06-08

### Changed

- Capped the supported Python range at `>=3.12,<3.14`. It was previously unbounded (`<4.0`), which advertised
  support for interpreter versions the project had never been tested against.

## [0.1.2] - 2026-06-08

### Changed

- Version bump only, to correct the metadata of the published package. No functional change.

## [0.1.1] - 2026-06-08

The release that turned a folder of shell scripts into an installable Python package.

### Added

- **Installable distribution.** The tool is now a `uv`/`hatchling` package (`uv tool install mega-snake`)
  instead of scripts sourced from a clone, with `mgsnake shell-path` bootstrapping the shell integration.
- **Maven support**: `set-maven` and `maven-project-setup`, aligned with the existing Gradle flow.
- **Command aliases** across the whole CLI, and a local developer config file (`init-local-config`) that is
  ignored by git.

### Changed

- Commands moved to kebab-case (`set-java`, `remote-branches-cleanup`), with the previous names kept as
  aliases.
- The package was renamed from `codename_snake` to `mega_snake`, and the source moved from `py/` to
  `src/mega_snake/`.
- Tooling migrated to `uv` and `ruff`.

### Removed

- The `gcloud` module was deferred out of the distribution to `offproject/`; it is not part of the published
  package.

## [0.1.0] - 2026-06-08

First publication to PyPI, of the tool as it existed before the packaging work in `0.1.1`. It is
recorded here for completeness: everything below was replaced one release later, so `0.1.0` is only
useful as the description of where the project started.

At this point `mgsnake` was a repository you cloned, not a package you installed:

- **Installation was `source`, not `install`.** `config_setup.sh` / `config_setup.ps1` were sourced
  from the clone, exported `WS_CONFIG_HOME` pointing back at it, and activated a virtual environment
  declared in `config.properties`. There was no `mgsnake` executable — the shell functions invoked
  `python3 -m <module>` directly, with `PYTHONPATH` set to the checkout.
- **Half the tool was shell.** `src/` held the commands as standalone scripts (`javaSet.sh`,
  `gradleSet.sh`, `expiredCertsJks.sh`, `gcloudSet.sh`, `parseJsonLogs.sh`, `formatting.sh`), while
  the Python half lived in `py/` under the `codename_snake` name, with its own nested
  `py/pyproject.toml`.
- **The command set was already recognisable**: environment configuration, `diff_tree`,
  `create_release`, `remote_branches` and `branch_cleanup` — under snake_case names, without aliases.
- Linting was `pylint` (`pylintrc`), and `combine.sh` concatenated the sources into a single file for
  pasting into an LLM.

Everything in this list is gone: `0.1.1` replaced the sourced-clone model with an installable
distribution, moved the shell commands into Python under `src/mega_snake/`, and renamed the package.
